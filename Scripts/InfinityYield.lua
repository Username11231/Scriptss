...
if makefolder and isfolder and writefile and isfile and readfile then
	pcall(function()
		for _, folder in {"infiniteyield", "infiniteyield/configs"} do
			if not isfolder(folder) then
				makefolder(folder)
			end
		end
...
CMDs[#CMDs + 1] = {NAME = 'loadconfig [configname]', DESC = 'Вгружает конфиг'}
CMDs[#CMDs + 1] = {NAME = 'saveconfig [configname]', DESC = 'Сохраняет конфег'}
CMDs[#CMDs + 1] = {NAME = 'listconfigs', DESC = 'выводит все конфиги'}
...
Players.LocalPlayer.OnTeleport:Connect(function(State)
	if KeepInfYield and (not TeleportCheck) and queueteleport then
		TeleportCheck = true
		queueteleport(loadstring(game:HttpGet("https://raw.githubusercontent.com/ScripterNumber/SPVKHUB/refs/heads/main/InfaWithConfig"))())
	end
end)
...
addcmd('saveconfig',{},function(args,speaker)
	if #args<1 then
		notify('Save Config','Usage: saveconfig <name>')
		return
	end
	
	if not writefile or not makefolder or not isfolder then
		notify('Error','File functions not supported')
		return
	end
	
	pcall(function()
		if not isfolder("infiniteyield") then
			makefolder("infiniteyield")
		end
		if not isfolder("infiniteyield/configs") then
			makefolder("infiniteyield/configs")
		end
	end)
	...
	pcall(function()
		if PrefixBox and PrefixBox.Text then
			table.insert(configContent, "prefix "..PrefixBox.Text)
		end
	end)
	
	pcall(function()
		if saves and saves.binds then
			for _,bind in ipairs(saves.binds) do
				table.insert(configContent, "bind "..bind[1].." "..bind[2])
			end
		end
	end)
		...
		if customAlias then
			for alias,cmd in pairs(customAlias) do
				if cmd and cmd.NAME then
					table.insert(configContent, "alias "..alias.." "..cmd.NAME)
				end
			end
		end
	end)
	
	local filePath = "infiniteyield/configs/"..args[1]..".txt"
	local fileData = table.concat(configContent, "\n")
	...
	if success then
		notify('Success','Saved: '..args[1])
	else
		notify('Error', tostring(err))
	end
end)

addcmd('loadconfig',{},function(args,speaker)
	if #args<1 then
		notify('Load Config','Usage: loadconfig <name>')
		return
	end
	...
	if not isfile(filePath) then
		notify('Error','Config not found: '..args[1])
		return
	end
	...
	if not success then
		notify('Error', 'Could not read config')
		return
	end
	
	for line in content:gmatch("[^\r\n]+") do
		if line ~= "" then
			execCmd(line, speaker)
		end
	end
	
	notify('Success','Loaded: '..args[1])
end)

addcmd('listconfigs',{},function(args,speaker)
	if not listfiles or not isfolder then
		notify('Error','listfiles not supported')
		return
	end
	
	if not isfolder("infiniteyield/configs") then
		notify('Info','No configs folder found')
		return
	end
	
	local success, files = pcall(listfiles, "infiniteyield/configs")
	
	if success and files then
		notify('Configs', #files..' config(s) found')
	else
		notify('Info','No configs found')
	end
end)
...
addcmd("savegame", {"saveplace"}, function(args, speaker)
    ...
    local Options = { ['NilInstances'] = true, ['IgnoreNotArchivable'] = true, ['IsolateLocalPlayer'] = false, ['IsolateLocalPlayerCharacter'] = false, ['RemovePlayerCharacters'] = false, ['IsolatePlayers'] = false } -- Documentation here https://luau.github.io/UniversalSynSaveInstance/api/SynSaveInstance
...
addcmd('fov',{},function(args, speaker)
	local fov = args[1] or 105
	if isNumber(fov) then
		workspace.CurrentCamera.FieldOfView = fov
	end
end)

addcmd('fovhook',{},function(args, speaker)
	local fov = args[1] or 105
	if isNumber(fov) then
        local Camera = workspace.CurrentCamera
		getgenv().hooked_fov = fov
        Camera.FieldOfView = getgenv().hooked_fov
        if getgenv().hooked_fov_bool ~= nil then
            return
        end
        getgenv().hooked_fov_bool = true

        local oldNewIndex
        oldNewIndex = hookmetamethod(game, "__newindex", function(self, key, value)
            if self == Camera and key == "FieldOfView" then
                value = getgenv().hooked_fov
            end
...
addcmd("secdex", {"secureddex"}, function(args, speaker)
    ...
    local HiddenInstances = setmetatable({}, {__mode = "k"})

    local function GetRandomName()
        local s = ""
        for i = 1, 16 do s = s .. string.char(math.random(97, 122)) end
        return s
    end

    local coreGui = cloneref(realgame:GetService("CoreGui"))
    local dexContainer = Instance.new("Folder")
    dexContainer.Name = GetRandomName()
    pcall(function() dexContainer.Parent = (gethui and gethui()) or coreGui end)
    HiddenInstances[dexContainer] = true

    local spoofScript = Instance.new("LocalScript")
    spoofScript.Name = "\0"
    spoofScript.Parent = dexContainer
    HiddenInstances[spoofScript] = true

    local function CreateVirtualGame(originalGame)
        local proxy = newproxy(true)
        local mt = getmetatable(proxy)
        
        mt.__index = function(self, key)
            local value = originalGame[key]
            ...
            if type(value) == "function" then
                return function(_, ...)
                    ...
                    if method == "GetDescendants" or method == "GetChildren" then
                        local res = value(originalGame, unpack(args))
                        if type(res) == "table" then
                            ...
                            for i = 1, #res do
                                ...
                                if cName == "NetworkClient" or cName == "NetworkReplicator" then continue end
                                if obj.Parent and obj.Parent.Name == "Chat" and cName == "BoolValue" then continue end
                                if HiddenInstances[obj] then continue end
                    ...
                    return value(originalGame, unpack(args))
                end
            end
            return value
        end
        ...
        mt.__newindex = function(self, key, val)
            originalGame[key] = val
        end
        
        mt.__tostring = function() return "DataModel" end
        mt.__metatable = "The metatable is locked"
        
        return proxy
    end

    local VirtualGame = CreateVirtualGame(realgame)

    local realInstanceNew = Instance.new
    local customInstance = {}
    customInstance.new = function(className, parent)
        local inst = realInstanceNew(className)
        HiddenInstances[inst] = true
        
        if className == "SelectionBox" or className == "Highlight" or className == "BoxHandleAdornment" then
            pcall(function() inst.Parent = dexContainer end)
            return inst
        end
    ...
    setmetatable(customInstance, { __index = Instance, __newindex = function() end })

    local function RunSecuredDex(dexSourceCode)
        local func, err = loadstring(dexSourceCode)
        if not func then return end

        local exploitEnv = getfenv(0)
        local realEnv = getrenv()

        local fakeEnv = setmetatable({
            script = spoofScript,
            Instance = customInstance,
            gethui = function() return dexContainer end,
            protectgui = function() end,
            cloneref = function(obj) return obj end,

            game = VirtualGame,
            
            newproxy = function(needMeta)
                local obj = {}
                if needMeta then setmetatable(obj, {}) end
                return obj
            end,

            getfenv = function(lvl)
                local e = getfenv(lvl or 1)
                if type(e) == "table" and e.script == spoofScript then return realEnv end
                return e
            end,
            ...
            __index = function(_, key)
                return exploitEnv[key] or realEnv[key]
            end,
        ...
        task.defer(function() pcall(func) end)
    end
    ...
    pcall(function()
        ...
        local oldFire; oldFire = hookfunction(bindable.Fire, function(self, ...)
            if not checkcaller() then
                local args = {...}
                if type(args[1]) == "table" then
                    for k, v in pairs(args[1]) do
                        if type(k) == "userdata" and getmetatable(k) then
                            return
                        end
                    end
                end
        ...
        local oldDestroy; oldDestroy = hookfunction(LocalPlayer.Destroy, function(self, ...)
        ...
        local oldKick; oldKick = hookfunction(LocalPlayer.Kick, function(self, ...)
            if not checkcaller() and self == LocalPlayer then
                return
            end
            return oldKick(self, ...)
    ...
    local function RunSecuredDex(dexSourceCode)
        ...
        local fakeEnv = setmetatable({
            ...
            __index = function(_, key) return exploitEnv[key] or realEnv[key] end,
            __newindex = function(_, key, val) exploitEnv[key] = val end
        })
    ...
    local dexSource = game:HttpGet("https://raw.githubusercontent.com/Username11231/Scriptss/refs/heads/main/Scripts/SecuredDex.lua")
    RunSecuredDex(dexSource)
...
addcmd('secrspy',{'securedremotespy'},function(args, speaker)
notify("Loading",'Hold on a sec')
...
local HiddenInstances = setmetatable({}, {__mode = "k"})
local HiddenFunctions = setmetatable({}, {__mode = "k"})
...
pcall(function()
        ...
        if not checkcaller() then
            ...
            if type(args[1]) == "table" then
                for k, _ in pairs(args[1]) do
                    if type(k) == "userdata" and getmetatable(k) then return end
                end
            end
        end
        return oldFire(self, ...)
    ...
    local oldDestroy; oldDestroy = hookfunction(LocalPlayer.Destroy, function(self, ...)
        if not checkcaller() and self == LocalPlayer then return coroutine.yield() end
        return oldDestroy(self, ...)
    end)
...
local targetGui = (gethui and gethui()) or cloneref(game:GetService("CoreGui"))
...
local realInstanceNew = Instance.new
local customInstance = setmetatable({
    new = function(className, parent)
        local inst = realInstanceNew(className, parent)
        HiddenInstances[inst] = true
        return inst
    end
}, {
...
if getgc then
    local oldGC = getgc
    getgc = newcclosure(function(inc)
        local res = oldGC(inc)
        local clean = {}
        for i = 1, #res do
            local v = res[i]
            if type(v) == "function" and HiddenFunctions[v] then continue end
            
            if type(v) == "table" and (rawget(v, "GenScript") or rawget(v, "Remote") or v == HiddenInstances) then continue end 
...
if getinstances then
    local oldGI = getinstances
    getinstances = newcclosure(function()
        local res = oldGI()
        local clean = {}
        for i = 1, #res do
            if not HiddenInstances[res[i]] then clean[#clean + 1] = res[i] end
        end
        return clean
    end)
end
...
if debug and debug.getinfo then
    ...
    debug.getinfo = newcclosure(function(func, ...)
        if HiddenFunctions[func] then
            local fakeInfo = oldGetInfo(print, ...) 
            fakeInfo.name = "core_execution"
            fakeInfo.source = "=[C]"
            fakeInfo.short_src = "[C]"
            fakeInfo.what = "C"
            return fakeInfo
        end
        return oldGetInfo(func, ...)
    end)
end

-- =====================================================================
-- ФАЗА 4: ЗАПУСК SIMPLE SPY
-- =====================================================================
local function RunSecuredSpy(spySourceCode)
    local func, err = loadstring(spySourceCode)
    if not func then 
        return warn("[AETHER] Compilation Failed:", err) 
    end
    ...
    local fakeEnv = setmetatable({
        script = spoofScript,
        Instance = customInstance,
        gethui = function() return targetGui end,
        protectgui = function(gui) HiddenInstances[gui] = true end,
        ...
        cloneref = function(obj)
            local s, r = pcall(cloneref, obj)
            return s and r or obj
        end,
        ...
        loadstring = function(code)
            ...
            if type(loaded) == "function" then HiddenFunctions[loaded] = true end
            return loaded
        end
    }, {
        __index = function(_, key)
            local val = exploitEnv[key]
            if val ~= nil then
                if type(val) == "function" then HiddenFunctions[val] = true end
                return val
            end
            local rVal = realEnv[key]
            if type(rVal) == "function" then HiddenFunctions[rVal] = true end
            return rVal
        end,
        __newindex = function(_, key, val)
            if type(val) == "function" then HiddenFunctions[val] = true end
    ...
    setfenv(func, fakeEnv)
    HiddenFunctions[func] = true
    
    local success, runtimeErr = pcall(func)
    if not success then
        warn("[AETHER] SimpleSpy internal runtime error:", runtimeErr)
    end
end

local spySource = game:HttpGet("https://raw.githubusercontent.com/78n/SimpleSpy/main/SimpleSpySource.lua")
RunSecuredSpy(spySource)
end)
